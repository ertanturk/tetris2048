...
def _pixel(x: float, y: float) -> None:
    ...
    xs = _scaleX(x)
    xy = _scaleY(y)
    pygame.gfxdraw.pixel(_surface, int(round(xs)), int(round(xy)), _pygameColor(_penColor))
...
def point(x: float, y: float) -> None:
    ...
    if _penRadius <= 1.0:
        ...
        pygame.draw.ellipse(
            ...
            pygame.Rect(xs - _penRadius, ys - _penRadius, _penRadius * 2.0, _penRadius * 2.0),
            0,
        )
...
def line(x0: float, y0: float, x1: float, y1: float) -> None:
    ...
    y1s = _scaleY(y1)
    pygame.draw.line(
        _surface, _pygameColor(_penColor), (x0s, y0s), (x1s, y1s), int(round(lineWidth))
    )
...
def filledCircle(x: float, y: float, r: float) -> None:
    ...
    if (ws <= 1.0) and (hs <= 1.0):
        ...
        ys = _scaleY(y)
        pygame.draw.ellipse(
            _surface, _pygameColor(_penColor), pygame.Rect(xs - ws / 2.0, ys - hs / 2.0, ws, hs), 0
        )
...
def polygon(x: list[float], y: list[float]) -> None:
    ...
    pygame.draw.polygon(_surface, _pygameColor(_penColor), points, int(round(_penRadius)))
